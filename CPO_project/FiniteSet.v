...
Proof. intros. pose proof (all_el Afin). induction l. right. intro. contradict H0.
  ...
  intro. destruct H0. symmetry in H0. now contradict H0. now contradict H0.
Qed.
...
Next Obligation.
  ...
  destruct (eq_dec bool_fin (a x) (b x)) eqn : AB. assumption. contradict H.
  intuition.
  + right. intro. assert (forall x, a x = b x). intro. now rewrite H. contradict EQ.
...
Next Obligation.
  ...
  + left. apply functional_extensionality. rewrite forallb_forall in EQ.
  intro. specialize EQ with x. pose proof (all_el Afin). apply EQ in H. unfold eq_bool in H.
  destruct (eq_dec Bfin (a x) (b x)) eqn : AB. assumption. contradict H.
  intuition.
  + right. intro. assert (forall x, a x = b x). intro. now rewrite H. contradict EQ.
    apply Bool.not_false_iff_true. apply forallb_forall. intro x. intro. unfold eq_bool.
...
Proof. intro. destruct a. cbn. reflexivity. Qed.
  ...
  Next Obligation. destruct a as [a Pa]. destruct b as [b Pb].
   ...
   + left. induction e.
   unfold Bool.Is_true in Pa, Pb. pose proof (Is_true_proof_irrelevance (P a) Pa Pb).
   rewrite <- H. reflexivity.
   + right. intro. inversion H. contradiction.
   Qed.
  Next Obligation.
    assert (forall (l : list A) x, match (@In_is_decidable A Afin l (proj1_sig x)) with
      | left Px => In x (build_subset l)
      | right _ => True
      end).
    - intros l x. destruct (In_is_decidable l (proj1_sig x)).
      -- induction l. contradict i. cbn. apply in_or_app.
         ...
         assert (((if P a0 as b return (P a0 = b -> list subAP)
             ...
             assert ((if P a0 as b return (P a0 = b -> list subAP)
               ...
               destruct x. cbn in *. induction H.
               now rewrite Is_true_proof_irrelevance with (P a0) i0 (Bool.Is_true_eq_left (P a0) Pa0).
