...
\dam{ici pourrait le résumé fichier par fichier de la lib}

\dam{ensuite c'est ici qu'on sort de l'intro et qu'on rentre dans le vif du sujet
  -> je mettrais une section
}
...
\dam{mettre une coercion sur la projection TBody permettrait d'alléger les notations}
...
\dam{variant iteres équivalent à \code{fun x => exists n, x = itere F n bot}, qui est sûrement plus lisible}
\dam{et je ne comprends pas la définition a; pourquoi ce n'est pas juste \code{sup (iteres F)}?
  ah oui, il faut montrer que (iteres F) est dirigé...
  -> dire que le second underscore est là pour ça, et que la preuve est omise
}
\begin{coq}
...
En revanche, dans un ensemble de valeurs de vérité quelconque $B$ (typiquement \code{bool}), on ne peut plus définir l'ensemble \code{iteres} $= \{F^n(\bot) ~ | ~ n \in \mathbb{N} \}$ \dam{``au type $X\to B$'' (le reste de la phrase me semble faux)}comme une simple proposition \code{forall n, iteres F (itere F n bot)}%, ie $\forall n \in \mathbb{N}, F^n(\bot) \in$ \code{iteres}. 
