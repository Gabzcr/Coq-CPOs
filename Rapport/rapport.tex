...
et les algèbres relationnelles \cite{relalg}. Ces projets pourraient bénéficier d'une bibliothèque générale, polyvalente et modulaire, dans laquelle les principaux résultats auraient déjà été formalisés sous leur forme la plus générale et réutilisable, adaptée à plusieurs niveaux de structures.
...
Contrairement à ce qui y est écrit, sous les hypothèses du théorème III, $F$ n'a pas forcément de point fixe \emph{minimal}. En particulier, le point fixe donné n'est pas minimal en général, même si c'est bien un point fixe.%, et en particulier le top de $P_0$ n'est pas un point fixe minimal en général, même si c'est bien un point fixe.
...
Un treillis complet typique est le treillis $(\mathcal{P}(X), \subseteq, \bigcup)$ des parties d'un ensemble $X$. Une question se pose alors, quel type choisir pour représenter les sous-ensembles ? Une partie $S \subseteq X$ n'est autre qu'une sélection d'éléments dans $X$, donc deux options naturelles pour le formaliser sont soit par la fonction caractéristique $\mathbb{1}_S$ de type \code{X -> bool}, soit par le prédicat d'appartenance à $S$ de type \code{X -> Prop}. Les deux options sont problématiques.

\begin{itemize}
\item[•] Le premier choix nous restreint à des ensembles décidables et ne permet pas de traiter le cas où $X$ est infini. %Le second choix est le seul qui permet de définir l'ensemble \code{Prop} comme un treillis complet dont l'ordre $P \leq Q$ est donné par $P \rightarrow Q$ et le sup d'un ensemble $\mathcal{P}$ est donné par $\bigvee \mathcal{P} = \exists P \in \mathcal{P}, P$. Car on doit par exemple pouvoir y définir le sous-ensemble des propositions décidables, qui est lui même indécidable (problème de la décision).
\item[•] Le deuxième choix ne permet pas de formaliser le treillis complet à deux éléments constitué des booléens $\mathbb{2}$ \code{= \{0,1\}} muni de l'ordre \code{0} $\leq$ \code{1}. Il nous faudrait définir sur cette structure une fonction \code{sup} de type $(\mathbb{2} \rightarrow$ \code{Prop}) $\rightarrow \mathbb{2}$. C'est problématique : pour une fonction \code{S : $\mathbb{2}$ -> Prop},  calculer \code{sup S} nécessite de décider \code{S 1} qui est une proposition arbitraire. En effet, le sup vaut 1 si $1 \in S$ et 0 sinon. Or on ne sait pas décider une proposition en général.
...
Mais ne sait pas déterminer en dehors d'un environnement de preuve lequel des deux côté du $\lor$ est vérifié. Nous aurions plutôt besoin de l'axiome suivant, bien plus fort, et qui permet de procéder par étude de cas pour définir des fonctions.
...
%Ici, le problème vient donc des valeurs de vérité que nous avons choisies pour définir nos sous-ensembles, et donc pour définir notre treillis et son sup. Sur le cas précis des booléens $\mathhbb{2}$ en tant que treillis complet, nous voudrions travailler dans \code{bool} au lieu de \code{Prop} et définir les sous-ensembles comme des fonctions \code{S : X -> bool}. Mais une telle définition ne fonctionnerait plus pour définir le treillis des propositions, par exemple.
...
Ensuite, on souhaite doter B d'un élément \code{Faux} noté \code{BFalse}, et des opérations classiques \og ou\fg{}, \og et\fg{} et l'implication : $\lor$, $\land$ et \code{->}, de manière à ce que qu'ils se comportent comme attendu avec l'évaluation \code{is\_true}. Par exemple pour \code{BFalse} et $\land$, ça donne :

\begin{coq}
  BFalse : B;
  BFalse_spec : not (is_true BFalse);
  BAnd : B -> B -> B;
...
La tactique \code{unfold\_spec} a précisément été créée dans la bibliothèque (\code{CPO.v}, ligne 63) pour pousser l'évaluation aux feuilles et se ramener à la logique habituelle dans \code{Prop}.
...
\begin{coq}
 Definition E_Y_z (Y : X -> B) (z : X) (* z sera bot *) (x0 : X) :=
   ...
   /\ (forall x, Y x -> Y (h x))  (* Y h-invariant (h bien defini sur Y) *)
   /\ Y z. (* reduit tout à l'ensemble vide si z n'est pas dans Y *)
...
Pendant le développement de la bibliothèque, nous avons exploré d'autres options qui pouvaient paraître plus naturelles au premier abord, mais se sont avérées problématiques. Je me propose dans cette section d'expliquer les leçons qu'on en a tirées. %d'en dépeindre maintenant les grandes étapes rapidement.
...
Ces types sont les types valides de notre structure de vérité booléenne, que nous pouvons donc enfin définir. Ci-dessous, \code{is\_member x l} est une fonction booléenne qui évalue si x est contenu dans l. Elle a été définie plus tôt dans le fichier. Et \code{el (projT2 X)} est la liste finie énumérant les éléments de X.
...
Maintenant, il devient possible et même facile de définir un ensemble à deux éléments, comme \code{bool}, en tant que CPO. Or, comme nous l'avons vu \ref{universalite}, C'est une définition qu'il n'était pas possible de faire avec \code{Prop}.

\begin{coq}
Program Instance B_CPO_bool : B_CPO B_PO_bool :=   {| sup D := D true; |}.
\end{coq}

On peut aussi par exemple définir un CPO à trois éléments : $\bot \leq x_1 \leq x_2$; en ajustant simplement la définition du sup :
...
\label{memo}
Notez la présence d'un opérateur \code{memo X} dans la définition de \code{B}. Cette fonction (de type \code{(X -> B) -> X -> B}) est une opération de mémoïsation sur X. Elle est utilisée pour améliorer le temps de calcul concret d'un point fixe minimal donné par le théorème II. En effet, comme on représente les ensembles par des fonctions \code{X -> B}, on peut avoir besoin de recalculer les sous-ensembles à chaque fois qu'on fait appel à eux, par exemple pour tester une appartenance.

La mémoïsation est une façon d'introduire des structures de données classiques (efficaces) dans un langage qui utilise plutôt des représentations très mathématiques. En l'occurrence, on pré-calculer et stocke un sous-ensemble \code{P : X -> B} dans une liste pour n'avoir plus besoin de tester que l'appartenance à la liste. Plus précisément, on stocke les valeurs de vérité de \code{P} sur chaque élément du CPO, c'est à dire les résultats du booléen \code{P x} pour tout $x \in X$.
...
%L'algorithme d'énumération est constitué des fonctions \code{add\_last\_image}, \code{build\_fun\_opt} et \code{build\_fun} (lignes 116-151). Sa preuve de correction est une obligation de la définition de l'ensemble fini des fonctions, lignes 169 à 230, et s'appuie sur plusieurs lemmes précédents.
...
on parcourt chacun des éléments de $Y$ (i.e. les éléments $x$ de $X$ fini, énumérés dans sa liste, qui satisfont la propriété \code{Y x}) et on maintient à jour une liste \code{candidats} des éléments maximaux rencontrés. Pour ce faire, à chaque nouvel élément examiné, on enlève de la liste tous les éléments qui lui sont inférieurs. Et, s'il n'y a pas d'élément plus grand que lui dans la liste de \code{candidats}, on l'ajoute.
